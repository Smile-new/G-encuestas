...
<head>
    ...
    <!-- Fuentes -->
    <link href="https://fonts.googleapis.com/css?family=Muli:300,300i,400,400i,600,600i,700,700i|Comfortaa:300,400,700" rel="stylesheet">
    ...
    <style>
        ...
        .font-large-1 { font-size: 1.25rem !important; }
        ...
        .pager ul.pagination li { margin: 0 2px; }
        ...
        #preguntasRespondidasContainer h6 { font-weight: 700; margin-top: 15px; color: #333; }
        ...
        #preguntasRespondidasContainer .respuesta-item p { margin-bottom: 0; font-size: 0.95rem; }
    </style>
</head>
                ...
                <ul class="nav navbar-nav float-right">
                    <li class="dropdown dropdown-user nav-item">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            ...
                            <a class="dropdown-item" href="/controlador/perfil"><i class="ft-user"></i> Editar Perfil</a>
                            <div class="dropdown-divider"></div>
                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-content collapse show">
                            <div class="card-body">
                                <p>
                                    Supervise las encuestas completadas, incluyendo la ubicación de captura para control de calidad.
                                    Mostrando <strong><?= count($listaRespuestas) ?> de <?= $totalRespuestas ?></strong> encuestas en total.
                                </p>
                                        ...
                                        <?php if (!empty($listaRespuestas)) : ?>
                                            <?php foreach ($listaRespuestas as $respuesta) : ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <button class="btn btn-primary btn-sm" onclick="mostrarDetalleRespuesta('<?= esc($respuesta['id_encuesta_realizada']) ?>')">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-lg-5 col-md-12 border-right">
                                ...
                                <p class="text-muted mt-2"><small>El mapa muestra la <strong>última ubicación</strong> registrada del encuestador (punto GPS) al momento de la captura.</small></p>
                            </div>
...
<script>
    ...
    function updateMapUI(direccion, lat, lng) {
        ...
        // Validar carga de Google Maps API
        if (typeof google === 'undefined' || typeof google.maps === 'undefined') {
            mapDiv.innerHTML = '<div class="map-placeholder-content"><p class="text-warning"><i class="la la-warning font-large-2 mb-2"></i><br> Esperando API de Google Maps...</p></div>';
            setTimeout(() => updateMapUI(direccion, lat, lng), 1500);
            return;
        }

        // Validar coordenadas
        if (!lat || !lng || isNaN(parseFloat(lat)) || isNaN(parseFloat(lng))) {
            mapDiv.innerHTML = '<div class="map-placeholder-content"><p class="text-info"><i class="la la-map-marker font-large-2 mb-2"></i><br> Ubicación GPS no registrada.</p></div>';
            return;
        }
        ...
        if (latFloat < -90 || latFloat > 90 || lngFloat < -180 || lngFloat > 180) {
            mapDiv.innerHTML = '<div class="map-placeholder-content"><p class="text-danger"><i class="la la-close font-large-2 mb-2"></i><br> Coordenadas GPS inválidas.</p></div>';
            return;
        }
        ...
        try {
            ...
            currentInfoWindow.setContent(
                `<div style="padding: 10px; max-width: 250px;">
                    <h6 style="margin: 0 0 8px 0; color: #333;">Ubicación Registrada</h6>
                    <p style="margin: 0; font-size: 12px; color: #666;">
                        <strong>Dirección:</strong> ${direccion || 'No disponible'}<br>
                        <strong>Coords:</strong> ${latFloat.toFixed(6)}, ${lngFloat.toFixed(6)}
                    </p>
                </div>`
            );
            ...
            // Refrescar tamaño del mapa una vez visible
            setTimeout(() => {
            ...
            mapDiv.innerHTML = '<div class="map-placeholder-content"><p class="text-danger">Error al cargar el mapa.</p></div>';
        }
    }
    ...
    $('#modalDetalleRespuesta').on('shown.bs.modal', function () {
        setTimeout(() => {
            if (mapInstance) {
                google.maps.event.trigger(mapInstance, 'resize');
                const { latitud, longitud } = currentMapData;
                if (latitud && longitud)
                    mapInstance.setCenter({ lat: parseFloat(latitud), lng: parseFloat(longitud) });
            } else {
                        ...
                        const html = `
                            <div class="respuesta-item">
                                ...
                                <p class="text-success text-bold-600">
                                    Respuesta: ${item.respuesta_seleccionada || 'Sin respuesta'}
                                </p>
                            </div>`;
