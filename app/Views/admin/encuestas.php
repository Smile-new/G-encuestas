...
<head>
    ...
    <title>Vota y Opina - Gestión de Encuestas</title>
    ...
    <style>
        ...
        .truncate-text {
            ...
            max-width: 200px;
            /* Ajusta este valor según cuánto texto quieres mostrar antes de truncar */
            display: inline-block;
            /* Necesario para que text-truncate funcione correctamente en td */
        }

        /* Ajuste específico para el título si necesita un max-width diferente o más pequeño */
        .truncate-title {
            max-width: 150px;
            /* Ajusta este valor según la longitud deseada para el título */
        }
                    ...
                    <div class="col-lg-12 grid-margin stretch-card">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <p class="card-description"> Aquí puedes gestionar todas las encuestas de tu sistema.
                                </p>

                                <a href="<?= base_url('encuestas/create'); ?>" class="btn btn-primary mb-3">Crear Nueva
                                    Encuesta</a>
                                            ...
                                            <?php if (!empty($encuestas) && is_array($encuestas)): ?>
                                                <?php foreach ($encuestas as $encuesta): ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <span class="truncate-text truncate-title"
                                                                title="<?= esc($encuesta['titulo']); ?>">
                                                                <?= esc($encuesta['titulo']); ?>
                                                            </span>
                                                        </td>
                                                        <td>
                                                            <span class="truncate-text"
                                                                title="<?= esc($encuesta['descripcion']); ?>">
                                                        ...
                                                        <td>
                                                            <div class="form-check form-switch">
                                                                ...
                                                                <label class="form-check-label"
                                                                    for="toggleStatus-<?= esc($encuesta['id_encuesta']); ?>">
                                                                    <span
                                                                        class="status-text <?= $encuesta['activa'] ? 'text-success' : 'text-danger'; ?>">
                                                        ...
                                                        <td>
                                                            ...
                                                            <a href="<?= base_url('encuestas/edit/' . $encuesta['id_encuesta']); ?>"
                                                                class="btn btn-warning btn-sm me-2" title="Editar Encuesta">
                                                                Editar
                                                            </a>
                                                            <form id="deleteForm-<?= esc($encuesta['id_encuesta']); ?>"
                                                                action="<?= base_url('encuestas/delete/' . $encuesta['id_encuesta']); ?>"
                                                                method="post" class="d-inline">
                                                                <?= csrf_field() ?>
                                                                <input type="hidden" name="_method" value="DELETE">
                                                                <button type="button" class="btn btn-danger btn-sm"
                                                                    onclick="confirmDelete(<?= esc($encuesta['id_encuesta']); ?>, '<?= esc($encuesta['titulo']); ?>')"
