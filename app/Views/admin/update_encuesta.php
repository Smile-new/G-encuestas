...
<head>
    ...
    <style>
        ...
        .question-card {
            ...
            /* Un borde más oscuro para distinguir las preguntas */
            padding: 1.5rem;
            margin-bottom: 1.5rem;
            border-radius: 8px;
            background-color: #2a2a2a;
            /* Fondo más oscuro para la tarjeta */
        }
        ...
        .option-item {
            ...
            align-items: center;
            gap: 10px;
            /* Espacio entre el input y el botón de eliminar */
            margin-bottom: 10px;
        }

        .option-item input {
            flex-grow: 1;
            /* Para que el input ocupe el espacio disponible */
        }

        .delete-btn-sm {
            padding: 0.3rem 0.6rem;
            /* Ajustar padding para botones pequeños */
            font-size: 0.75rem;
        }
        ...
        .btn-custom-margin {
            margin-right: 10px;
            /* Espacio entre botones si es necesario */
        }

        .text-danger {
            font-size: 0.875em;
            /* Tamaño de letra para mensajes de error */
        }
...
<body>
        ...
        <div class="main-panel">
                    ...
                    <div class="col-lg-12 grid-margin stretch-card">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <p class="card-description"> Edita los detalles de la encuesta
                                    "<?= esc($encuesta['titulo'] ?? '') ?>". </p>
                                ...
                                <form class="forms-sample"
                                    action="<?= base_url('encuestas/update/' . esc($encuesta['id_encuesta'])) ?>"
                                    method="post">
                                    <?= csrf_field() ?>

                                    <div class="form-group">
                                        <label for="titulo">Título de la Encuesta <span
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" id="descripcion" name="descripcion" rows="4"
                                            placeholder="Describe brevemente la encuesta"><?= old('descripcion', esc($encuesta['descripcion'] ?? '')) ?></textarea>
                                        <?php if (isset($errors) && array_key_exists('descripcion', $errors)): ?>
                                            <p class="text-danger mt-1">
                                                <?= esc($errors['descripcion']) ?>
                                            </p>
                                        <?php endif; ?>
                                    </div>
                                    ...
                                    <p class="card-description">Edita las preguntas y opciones existentes, o añade
                                        nuevas. Cada pregunta debe tener al menos dos opciones.</p>
                                    ...
                                    <div id="preguntas-container">
                                        ...
                                        if (!empty($displayQuestions)):
                                            foreach ($displayQuestions as $qIndex => $question):
                                                ...
                                                // EXTRAEMOS EL ID DE LA PREGUNTA (si existe)
                                                $idPregunta = isset($question['id_pregunta']) ? $question['id_pregunta'] : '';
                                                $displayOptions = isset($question['opciones']) ? $question['opciones'] : [];
                                                ?>
                                                <div class="question-card" data-question-index="<?= $qIndex ?>">
                                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                                        <h5 class="mb-0">Pregunta <span class="question-number">
                                                                <?= $qIndex + 1 ?>
                                                            </span></h5>
                                                        <button type="button"
                                                            class="btn btn-danger btn-sm remove-question-btn">Eliminar
                                                            Pregunta</button>
                                                    </div>

                                                    <input type="hidden" name="preguntas[<?= $qIndex ?>][id_pregunta]"
                                                        value="<?= $idPregunta ?>">

                                                    <div class="form-group">
                                                        <label>Texto de la Pregunta <span class="text-danger">*</span></label>
                                                        <input type="text" class="form-control"
                                                            name="preguntas[<?= $qIndex ?>][texto_pregunta]"
                                                            placeholder="Ej: ¿Cuál es su nivel de satisfacción?"
                                                            value="<?= $questionText ?>" required>
                                                        <input type="hidden" name="preguntas[<?= $qIndex ?>][tipo_pregunta]"
                                                            value="<?= esc($questionType) ?>">
                                                    </div>
                                                    ...
                                                    <div class="options-container">
                                                        ...
                                                        if (!empty($displayOptions)):
                                                            foreach ($displayOptions as $oIndex => $option):
                                                                ...
                                                                // EXTRAEMOS EL ID DE LA OPCIÓN (si existe)
                                                                $idOpcion = isset($option['id_opcion']) ? $option['id_opcion'] : '';
                                                                ?>
                                                                <div class="option-item" data-option-index="<?= $oIndex ?>">
                                                                    <input type="hidden"
                                                                        name="preguntas[<?= $qIndex ?>][opciones][<?= $oIndex ?>][id_opcion]"
                                                                        value="<?= $idOpcion ?>">

                                                                    <input type="text" class="form-control"
                                                                        name="preguntas[<?= $qIndex ?>][opciones][<?= $oIndex ?>][texto_opcion]"
                                                                        placeholder="Opción <?= $oIndex + 1 ?>"
                                                                        value="<?= $optionText ?>" required>
                                                                    <button type="button"
                                                                        class="btn btn-outline-danger btn-sm remove-option-btn delete-btn-sm">Eliminar</button>
                                                                </div>
                                                            <?php endforeach;
                                                        endif; ?>
                                                    </div>
                                                    <button type="button" class="btn btn-info btn-sm add-option-btn mt-2">Añadir
                                                        Opción</button>
                                                </div>
                                            <?php endforeach;
                                        endif; ?>
                                    ...
                                    <button type="button" class="btn btn-success mt-3" id="add-question-btn">Añadir
                                        Nueva Pregunta</button>

                                    <div class="mt-4">
                                        <button type="submit" class="btn btn-primary me-2 btn-custom-margin">Guardar
                                            Cambios</button>
            ...
            <footer class="footer">
                <div class="d-sm-flex justify-content-center justify-content-sm-between">
                    <span class="text-muted d-block text-center text-sm-left d-sm-inline-block">Copyright © 2025 Your
                        Company. All rights reserved.</span>
                </div>
            </footer>
        </div>
    </div>
    ...
    <template id="question-template">
        <div class="question-card">
            ...
            <div class="form-group">
                <label>Texto de la Pregunta <span class="text-danger">*</span></label>
                <input type="text" class="form-control" name="preguntas[idx_q][texto_pregunta]"
                    placeholder="Ej: ¿Cuál es su nivel de satisfacción?" required>
            ...
            <div class="options-container">
                ...
                <div class="option-item">
                    <input type="text" class="form-control" name="preguntas[idx_q][opciones][0][texto_opcion]"
                        placeholder="Opción 1" required>
                    <button type="button"
                        class="btn btn-outline-danger btn-sm remove-option-btn delete-btn-sm">Eliminar</button>
                </div>
                <div class="option-item">
                    <input type="text" class="form-control" name="preguntas[idx_q][opciones][1][texto_opcion]"
    ...
    <template id="option-template">
        <div class="option-item">
            <input type="text" class="form-control" name="preguntas[idx_q][opciones][idx_o][texto_opcion]"
                placeholder="Opción">
            <button type="button"
                class="btn btn-outline-danger btn-sm remove-option-btn delete-btn-sm">Eliminar</button>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            let questionIndex = preguntasContainer.children.length;
            ...
            function updateIndexes() {
                Array.from(preguntasContainer.children).forEach((qCard, qIdx) => {
                    ...
                    options.forEach((opt, oIdx) => {
                        ...
                        const oId = opt.querySelector('input[name*="[id_opcion]"]');
                        ...
                        if (oId) {
                            oId.name = `preguntas[${qIdx}][opciones][${oIdx}][id_opcion]`;
                        }
                    });
                });
            ...
            function addQuestion() {
                ...
                // Reemplazamos el marcador de posición por el índice actual
                const html = tmp.innerHTML.replace(/idx_q/g, questionIndex);

                // Insertamos el HTML procesado
                preguntasContainer.insertAdjacentHTML('beforeend', html);
                updateIndexes();
            }
            ...
            preguntasContainer.addEventListener('click', e => {
                ...
                if (target.classList.contains('remove-question-btn')) {
                    ...
                    if (preguntasContainer.children.length > 1) {
                        Swal.fire({
                            ...
                            text: 'Se eliminará la pregunta y sus opciones. Si ya hay respuestas registradas, podrían perderse.',
                            icon: 'warning',
                        ...
                        Swal.fire('Atención', 'La encuesta debe tener al menos una pregunta.', 'info');
                    }
                    return;
                }
                ...
                if (target.classList.contains('add-option-btn')) {
                    ...
                    // Reemplazamos índices de pregunta y opción
                ...
                if (target.classList.contains('remove-option-btn')) {
                    ...
                    if (realOptions > 2) {
                        ...
                        Swal.fire('Atención', 'Cada pregunta debe tener al menos dos opciones.', 'info');
                    }
                }
            });
            ...
            form.addEventListener('submit', e => {
                ...
                Array.from(preguntasContainer.children).forEach(qCard => {
                    ...
                    const optionInputs = Array.from(qCard.querySelectorAll('.options-container .option-item input[type="text"]'));

                    if (optionInputs.length < 2) {
                        Swal.fire('Error', 'Cada pregunta debe tener al menos dos opciones.', 'error');
                        valid = false;
                    }

                    optionInputs.forEach(inp => {
                        if (!inp.value.trim()) {
                            Swal.fire('Error', 'Todas las opciones deben tener texto.', 'error');
                            valid = false;
                        }
                    });
                });
